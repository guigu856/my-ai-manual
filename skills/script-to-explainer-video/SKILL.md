...
description: 把口播脚本/观点文案变成极简概念动效解说视频的完整生产线：先配音、以配音实测时长为全片主时钟，Remotion/HyperFrames 程序化概念动效，内嵌字幕、逐帧自检交付可观看成片
version: 1.0.0
---
...
用户只给脚本、未指定参数时，按下方「情境参数判断依据」自行判断执行，不要反问。不随情境变化的：先声音后画面、交付可观看成片（不是方案截图或素材包）。
...
- 概念元素用图标、形状、节点、路径、容器表达；**禁"文字+矩形外边框"卡片形态，圆角描边矩形也算**。
...
- 逐帧播放或按需抽帧查看，检查：错位/越界/偏线、文字重叠/裁切、鬼影、子元素提前显形、连线打架、交叠期元素叠加、元素占比、字幕单行与截断、标题出现时机。
...
- 用户想要特定真人音色（如剪映/抖音爆款声线）：该类音色无公开 API，**走剪映 SRT round-trip**——导出句级 SRT → 用户在剪映导入并配音 → 提取最终音频。
...
- 为什么自建 Remotion 流程而不是现成管线：观点类片视觉要求高度定制，程序化渲染才有逐帧控制力。
- 为什么音频主时钟：配音自然语速不可预知，先画面后配音必然导致动效与讲解脱节；先配音则一切节拍有据可依。
- 为什么中央构图、无常驻元素：常驻标志+跨场长线被用户强烈反感，且小屏上持续抢占视觉。
