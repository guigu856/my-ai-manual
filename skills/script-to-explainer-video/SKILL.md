...
8. **默认使用低状态生产剖面。** 观点口播、知识讲解和 3 分钟内概念视频默认使用页面隔离：一页一个判断、旧页状态退出、页面之间不共享复杂对象。只有跨场变形本身承担解释任务时才使用连续场景。
9. **执行阶段只有一个真源。** `render-plan.json` 是画面、字幕样式、页面边角信息和时间轴的唯一执行真源；组件不得另存第二套秒数、字幕样式或场景文案。
10. **程序通过不等于成片通过。** Lint、布局、对比度和媒体探测只证明技术可运行；风格统一、语义时序、边角标签、标题和字幕外观必须基于最终编码 MP4 单独审查。
...
- `references/visual-lock-and-complexity-budget.md`
...
- 需要保留或退出的元素
- 风险点
- 生产剖面：`page-isolated` 或 `continuous-diagram`
- 页面边角信息：左上角分类标签、右上角页码和主标题
- 场景背景 token、跨场保留对象和复杂度预算

此阶段只使用相对节拍或 cue 引用，不写最终绝对秒数。

**Gate 3：** 分镜已覆盖全部 Beat；画面不是字幕复述；每个 Scene 都能说明它帮助观众理解了什么；背景、字幕外观和页面边角信息已经锁定；默认剖面不存在无理由跨场对象。最终脚本与分镜在此阶段锁定。
...
禁止把 storyboard 中的估算时长直接当成最终帧号。渲染计划至少包含：生产剖面、画布、fps、全局 `style_lock`、Scene 区间、页面边角信息、State 区间、Event、字幕引用、资源引用和引擎适配信息。

生成后运行：

```bash
python scripts/validate_project.py --project <project>
```

先修复验证器报告的背景漂移、边角字段缺失、字幕边框违约、跨场残留、复杂度超限和时间轴错误，再进入实现。
...
- 从 `render-plan.json.style_lock` 读取背景、字幕和页面边角规则，不在 CSS、ASS 或组件中重新定义另一套值
- 布局和连接点由统一坐标与约束计算
- 颜色、字体、间距和 motion token 全片一致
- 先渲染关键状态 still，再渲染完整视频
- `page-isolated` 中每次换页必须清空上一页状态；禁止用降透明度残留模拟连续性
- 默认每 Scene 同时只保留一个主要焦点、最多四个内容组和一个主运动关系
...
1. 全片背景/色彩模式是否与 `style_lock` 一致，禁止无说明混用浅色和深色页面。
2. 逐 Scene 核对左上角分类标签、右上角页码和主标题：存在、可见、在画布内、互不重叠。
3. 从最终 MP4 抽取字幕帧，核对背景框、描边、阴影和安全区；不能只检查源 CSS 或 ASS 参数。
4. 抽取全部场景边界与高风险区，确认旧页对象已退出且信息没有抢跑。

输出 `qc/qc-report.md`、接触表、探测数据和 `final.mp4`。

**Gate 7：** 自动检查和最终像素审查分别给出结论；任一项为 FIX/BLOCK 都不得写 `PASS`。所有阻断项清零后才交付。

## 默认判断

| 参数 | 默认值 |
|---|---|
| 画幅 | 用户未指定时统一使用 7:5（默认画布 1400×1000，横向）；只有用户明确要求其他比例时才覆盖默认值 |
| 配音 | 用户未指定时使用剪映软件中的“真人口播女”音色；优先使用剪映直接生成或导出的该音色真实音频，不得静默替换成其他女声 |
| 帧率 | 30fps；高密度运动或特殊要求再提高 |
| 时长 | 由内容决定，不为了凑时长填充画面 |
| 风格 | 先从题材、受众和态度判断，不默认所有技术内容都使用深色科技风 |
| 生产剖面 | 3 分钟内观点口播/知识讲解默认 `page-isolated`；只有跨场对象变换承担解释任务时使用 `continuous-diagram` |
| BGM | 默认不添加；需要时确保版权并以不压人声为准 |
| 字幕 | 默认开启，按语义短语切分；默认无背景框、无描边、无阴影，用户或视觉方案明确要求时才覆盖 |
...
- `style_lock`、页面边角信息、字幕外观或生产剖面改变：实现、字幕渲染、QC 和成片全部失效。
