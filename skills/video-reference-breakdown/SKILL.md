...
description: 拆解和分析参考视频，自动处理本地文件、视频链接或平台分享文本，下载并固化视频，读取真实媒体信息，抽帧、生成联系表、分析声音、补充重点区间证据，最终交付普通用户也能看懂的四部分 DOCX 拆解报告。用户说“拆解这个视频”“分析参考片”“看看这个链接怎么剪的”“这条视频为什么好看”“提炼可复用剪法”时必须使用，即使没有点名本 Skill。视频制作、已有成片质量审查、只下载不分析、只提取字幕或只做格式转换走其他 Skill。
metadata:
  version: 1.0.0
...
输入本地视频、链接、分享文本或对话附件，输出一份连续可读的四部分 DOCX：

1. 第一部分：这条视频是怎么立起来的；
2. 第二部分：它用了什么素材和音乐；
3. 第三部分：它怎么一段段剪出来的；
4. 第四部分：如果重新做一条，应该学什么。
...
- `references/report-contract.md`：四部分标题、表格和每部分要求；
- `references/evidence-and-language.md`：事实、判断、证据和通俗表达纪律；
- `templates/report-outline.md`：写作骨架。

第三部分的时间线使用八列并连续覆盖全片。普通段使用易读粒度，复杂段拆细并在“画面处理”列写全素材使用、空间层级与遮挡、进入→保持→退出、效果与衔接。第四部分至少整理一套可以照着执行的连续剪法，不能只写“节奏感强”“转场自然”等空泛评价。
