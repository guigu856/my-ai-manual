...
约束对齐业界已验证问题：CSS 动画在逐帧渲染中不生效、未 clamp 导致飞出画面、`useCurrentFrame` 相对 Sequence 而非全局、硬切后静止蹦字、交互式 CLI 导致自动化失败等。
...
关键文字距左右 ≥ 64px，距上下 ≥ 72px。主标题必须在目标预览尺寸下可读。
